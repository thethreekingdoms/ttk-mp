...
console.warn('当前渲染模式版本：miniprogram-element@2.x 版本。\n\n2.x 版本对比 1.x 版本去除了渲染内置组件时额外引入的一层节点，此模式基于 2.11.2 基础库实现，如果在 2.11.2 版本之前的基础库环境运行，则会降级成 1.x 渲染模式。\n\n渲染过程如果升级版本过程中遇到样式错乱问题，可尝试去除使用 1.x 版本时额外追加的强依赖结构的兼容样式，也可选择退回 1.x 版本（退回版本可以使用 generate.renderVersion 和 generate.elementVersion 配置：https://jeffycai.github.io/kbone/docs/config/#generate-renderversion ，指定 tag 为 core-v1 即可）。')
